...
def main():
   # <prog> <git userid> <repo name> <release tag> <file name of asset to replace/add in release>
   # upload_asset.py  buildqa  tester1  2017-07-26  darwin.tgz
   usage_msg = "$ %s <release tag> <asset in release to delete and upload>" % (sys.argv[0])
   ...
   #
   # curl -sH 'Authorization: token 660f3de74159a58b5eca684f4b11f3cc341ddc8c' https://api.github.com/repos/buildqa/tester1/releases/tags/2018-07-26
   ...
   if (cnt_asset == 0):
      ...
      # Asset previously deleted, so no URL etc., returned for it/
      # Assume asset with same name must be (re)uploaded
      # 
      # sys.exit(1)
      upload_asset = True
   ...
   if asset_url and (cnt_asset != 0):
      ...
      #
      # curl '' -X DELETE -H 'Authorization: token 660f3de74159a58b5eca684f4b11f3cc341ddc8c' 'https://api.github.com/repos/buildqa/tester1/releases/assets/8018113'
   ...
   if upload_asset:
       if debug: print("Travis deploy stage should re-upload file %s") % asset_name
       # Add curl code here to re-upload asset with *new* asset ID ?
